...
public class StudentController {
    ...
    public ResponseEntity<CustomResponse> getStudent(
            @PathVariable
                    @NotNull(message = "ID field cannot be left empty")
                    @Positive(message = "ID must be a positive value")
                    Long studentId) {
        return ResponseEntity.ok(
                new CustomResponse(
                        "00", "Successful", studentService.getStudentAndMapToDto(studentId)));
    }

    @PatchMapping("/{studentId}")
    public ResponseEntity<CustomResponse> updateStudentDetails(
            @PathVariable
                    @NotNull(message = "ID field cannot be null")
                    @Positive(message = "ID ust be a positive value")
                    Long studentId,
            @RequestBody @Valid UpdateStudentDetailsRequest updateStudentDetailsRequest) {
        return ResponseEntity.ok(
                new CustomResponse(
                        "00",
                        "Successful",
                        studentService.updateStudentDetails(
                                studentId, updateStudentDetailsRequest)));
    }
    ...
    public ResponseEntity<CustomResponse> getAllStudents(
        ...
        return ResponseEntity.ok(
                new CustomResponse(
                        "00",
                        "Successful",
                        studentService.findAllStudents(department, level, size, page)));
    }
}
