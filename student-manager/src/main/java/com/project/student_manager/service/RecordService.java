...
public class RecordService {
    ...
    public void UpdateScore(Long studentId, Long courseId, UpdateScoreRequest updateScoreRequest){
        EnrollmentEntity enrollmentEntity = enrollmentService.findByStudentIdAndCourseId(studentId, courseId);
        RecordEntity recordEntity = recordEntityRepository.findByEnrollment_EnrollmentId(enrollmentEntity.getEnrollmentId())
                .orElseGet(()->{RecordEntity newRecordEntity = new RecordEntity();
                        newRecordEntity.setEnrollment(enrollmentEntity);
                return newRecordEntity;
                }
                );

        if (Objects.nonNull(updateScoreRequest.getCa())&&Objects.nonNull(updateScoreRequest.getExam())){
