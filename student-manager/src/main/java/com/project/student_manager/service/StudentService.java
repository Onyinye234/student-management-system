...
public class StudentService {
    ...
    public StudentEntity getStudentById(Long studentId) {
        return studentEntityRepository
                .findById(studentId)
                .orElseThrow(
                        () ->
    ...
    public StudentDto enrollStudent(StudentRegistrationRequest studentRegistrationRequest) {
        if (studentEntityRepository
                .findByEmail(studentRegistrationRequest.getEmail())
                .isPresent()) {
            throw new StudentManagerException(
                    ErrorType.STUDENT_ALREADY_REGISTERED, "Student already exists!");
        }
        if (studentEntityRepository
                .findByMatricNumber(studentRegistrationRequest.getMatricNumber())
                .isPresent()) {
            throw new StudentManagerException(
                    ErrorType.STUDENT_ALREADY_REGISTERED, "Student already exists!");
        }
    ...
    public StudentDto updateStudentDetails(
            Long studentId, UpdateStudentDetailsRequest updateStudentDetailsRequest) {
    ...
    public Page<StudentDto> findAllStudents(
            Department department, Level level, int size, int page) {
        Page<StudentEntity> students;
        Pageable pageable = PageRequest.of(page, size, Sort.by("fullName").ascending());
        if (department != null && level != null) {
            students =
                    studentEntityRepository.findByLevelAndDepartment(pageable, level, department);
        } else if (level != null) {
