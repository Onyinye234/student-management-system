...
public class StudentService {
    ...
    public StudentDto enrollStudent(StudentRegistrationRequest studentRegistrationRequest){
       if (studentEntityRepository.findByEmail(studentRegistrationRequest.getEmail()).isPresent()){
           throw new StudentManagerException(ErrorType.STUDENT_ALREADY_REGISTERED,"Student already exists");
       }
        if (studentEntityRepository.findByMatricNumber(studentRegistrationRequest.getMatricNumber()).isPresent()){
            throw new StudentManagerException(ErrorType.STUDENT_ALREADY_REGISTERED,"Student already exists");
        }

        StudentEntity studentEntity = new StudentEntity(studentRegistrationRequest.getFullName(), studentRegistrationRequest.getEmail(), studentRegistrationRequest.getMatricNumber(), studentRegistrationRequest.getLevel());
